...
// Element Class 
class PerchHeroImage extends PerchVcMap {
     
    private $base = 'perch_hero_image';

    private $title = 'Hero image';
    ...
    public function perch_image_output( $atts, $content = NULL ) {
        ...
        $html = apply_filters('perch_modules/hero_image/output', $html, $html_args, $atts);
    ...
    public function vc_mapping( $return = false ) {
       ...
       $vc_map = array(
                ...
                'base' => $this->base,
                'name' => $this->title,                
                'description' => __('Display hero image', 'perch'),                 
                'params' => $params,
    ...
    private function display_as(){
        return array(
            array(
                ...
                'value' => perch_numeric_dropdown_options( 'hero-', '-img', 'Hero image style ', array('limit' => 12) ),
                'std' => ''
            )
        );
    }

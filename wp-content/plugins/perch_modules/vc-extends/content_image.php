...
// Element Class 
class PerchContentImage extends PerchVcMap {
     
    private $base = 'perch_content_image';

    private $title = 'Content image';
    ...
    public function perch_image_output( $atts, $content = NULL ) {
        ...
        $html = apply_filters('perch_modules/content_image/output', $html, $html_args, $atts);  
    ...
    public function vc_mapping( $return = false ) {
       ...
       $vc_map = array(
                ...
                'base' => $this->base,
                'name' => $this->title,                
                'description' => __('Display content image', 'perch'),                 
                'params' => $params,
    ...
    private function display_as(){
        return array(
            array(
                ...
                'value' => perch_numeric_dropdown_options( 'content-', '-img', 'Content image style ', array('limit' => 9) ),
                'std' => ''
            )
        );
    }

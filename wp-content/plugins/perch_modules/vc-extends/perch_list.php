...
// Element Class 
class PerchList extends PerchVcMap {
     
    private $base = 'perch_list';

    private $title = 'List group';
    ...
    function __construct() {
        ...
        // Shortcode filter
        add_filter( $this->base, array( $this, 'perch_list_output' ), 30, 2 ); 
        add_filter( $this->base.'_slide', array( $this, 'perch_list_slide_output' ), 30, 2 ); 
    }
    ...
    private function vc_map_args(){        
        ...
        $array = array_merge($array, PerchVcMap::_vc_map_input_field_group('list_title', 'Title'));
        $array = array_merge($array, PerchVcMapListGroup::list_group_args());
    ...
    // Element HTML
    public function perch_list_output( $atts, $content = NULL ) {
        ...
        <div '. implode( ' ', $wrapper_attributes ).'> 
            '.perch_modules_get_vc_param_html('list_title', $atts, $map_arr ).'           
            '.PerchVcMapListGroup::list_group_html($atts).'     
        </div>';

        $html_args = array(
            'wrapper_attributes' => $wrapper_attributes,
            'title' => perch_modules_get_vc_param_html('list_title', $atts, $map_arr ),
            'list_group_html' => PerchVcMapListGroup::list_group_html($atts),
        );     

        $html = apply_filters('perch_modules/list/output', $html, $html_args, $atts); 
    ...
    public function perch_list_slide_output( $atts, $content = NULL ) {
        ...
        $html .= self::perch_list_output($atts);
        $html .='</div>';
